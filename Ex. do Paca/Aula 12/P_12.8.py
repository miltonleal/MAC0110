#Escreva uma função que compara duas datas que são dadas na forma dia, mês e ano. A função
# devolve 1 se a primeira data é maior que a segunda, 0 se são iguais e -1 se a segunda é maior que a primeira.
...
def ComparaData(dia_1, mes_1, ano_1, dia_2, mes_2, ano_2):
    ...
    while ano_1 != ano_2:
        ...
        if ano_2 > ano_1: return 1
        return -1

    while mes_1 != mes_2:

        if mes_2 > mes_1: return 1
        return -1

    while dia_1 != dia_2:

        if dia_2 > dia_1: return 1
        return -1
