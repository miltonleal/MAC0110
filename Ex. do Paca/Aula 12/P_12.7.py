...
#  acrescentando os parâmetros ps = prova substitutiva. Se ps >= 0, ps substitui uma das
# notas p1 ou p2 com o respectivo peso
...
def main ():
    ...
    print("A média final do aluno é: %2.1f" % media_final(p1, p2, ps, ep1, ep2, ep3))

def media_final (p1,p2, ps, ep1, ep2, ep3):

    p = (p1+(2*p2))/3
    ep = (ep1 + (2*ep2) + (3*ep3))/6

    if ps > 0:
        p = max((ps + 2*p2)/3, (p1+2*ps)/3)
